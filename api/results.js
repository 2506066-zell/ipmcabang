...
async function list(req, res) {
  ...
  // Simple ranking: Show ALL attempts, ordered by Score (DESC), then Time Taken (ASC), then Earliest (ASC)
  const queryText = `
    SELECT 
        id,
        created_at AS ts,
...
async function create(req, res) {
  ...
  const userAnswers = b.answers || {}; // { questionId: 'answerKey' }
  ...
  // Fetch correct answers for this set
  const questions = (await query`SELECT id, correct_answer FROM questions WHERE quiz_set=${quiz_set} AND active=true`).rows;

  if (!questions.length) {
    return json(res, 400, { status: 'error', message: 'Set soal tidak ditemukan atau tidak aktif.' });
  }
  ...
  const COOLDOWN_MS = 10 * 1000; // Reduced cooldown

  // Idempotency / Duplicate Check
  // Check if same user submitted same score for same set within last 10 seconds (double click protection)
  const recentDup = (await query`
    SELECT id FROM results 
    WHERE user_id=${userRow.id} 
    AND quiz_set=${quiz_set} 
    AND score=${score} 
  ...
  if (last && Number(last.finished_at || 0) > 0) {
    ...
    if (delta >= 0 && delta < COOLDOWN_MS) {
      return json(res, 429, { status: 'error', message: 'Terlalu cepat. Harap tunggu sebentar.' });
    }
  }
  ...
  const DAILY_LIMIT = 50; // Increased limit
